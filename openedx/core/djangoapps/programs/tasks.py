...
def get_completed_programs(site, student):
    """
    Determine which programs have been completed by a given user.
...
def get_certified_programs(student: User, raise_on_error: bool = False) -> List[str]:
    ...
    Args:
        student(User): User object representing the student

    Keyword Arguments:
        raise_on_error (bool): Reraise errors back to the caller, instead of returning empty results.

    Returns:
        list(str): UUIDs of the programs for which the student has been awarded a certificate
...
def award_program_certificates(self, username):  # lint-amnesty, pylint: disable=too-many-statements
    ...
    try:
        ...
        LOGGER.exception(f"Task award_program_certificates was called with an invalid username {username}")
    ...
    try:
        ...
        if not completed_programs:
            LOGGER.info(f"Task award_program_certificates was called for user {student} with no completed programs")
            return
    ...
    if new_program_uuids:
        ...
        for program_uuid in new_program_uuids:
            ...
            try:
                LOGGER.info(f"Visible date for user {student} : program {program_uuid} is {visible_date}")
                award_program_certificate(credentials_client, student, program_uuid, visible_date)
                LOGGER.info(f"Awarded a program certificate to user {student} in program {program_uuid}")
            except HTTPError as exc:
                if exc.response.status_code == 404:
                    LOGGER.exception(
                        f"Unable to award a program certificate to user {student} in program {program_uuid}. A program "
                        "certificate configuration for this program may not exist in Credentials"
                    )
                elif exc.response.status_code == 429:
                    rate_limit_countdown = 60
                    error_msg = (
                        f"Rate limited. Retrying task to award a program certificate to user {student} in program "
                ...
                LOGGER.exception(
                    f"Failed to award a program certificate to user {student} in program {program_uuid}"
                )
        ...
        if failed_program_certificate_award_attempts:
            ...
            # of attempts.
            error_msg = (
                f"Failed to award a program certificate(s) for user {student} in program(s) "
...
def update_credentials_course_certificate_configuration_available_date(
    ...
    """
    This task will update the CourseCertificate configuration's available date in Credentials. This is different from
    the "visible_date" attribute. This date will always either be the available date that is set in Studio for a given
    course, or it will be None.
...
def award_course_certificate(self, username, course_run_key):
    ...
    def _retry_with_custom_exception(username, course_run_key, reason, countdown):
        exception = MaxRetriesExceededError(
            f"Failed to award a course certificate for user {username} for course {course_run_key}. Reason: {reason}"
        )
        return self.retry(exc=exception, countdown=countdown, max_retries=MAX_RETRIES)

    LOGGER.info(f"Running task award_course_certificate for username {username} and course run {course_run_key}")
    ...
    try:
        ...
        LOGGER.exception(f"Task award_course_certificate was called with an invalid username {username}")
        return

    try:
        course_key = CourseKey.from_string(course_run_key)
        # Attempt to retrieve the course overview for the given course run key, this is required to set the
        # "visible date" of the course certificate being awarded on the Credentials side. If no course overview can
        # be retrieved, just return. There is no point in retrying.
        course_overview = get_course_overview_or_none(course_key)
        if not course_overview:
            LOGGER.warning(
                f"Unable to award a course certificate to user {user} in course run {course_run_key}. A course "
                "overview for the given course run key could not be found"
            )
            return

        # get the course certificate instance for the retrieved user and course run key if it's both passing and
        # available in a supported mode
        try:
            certificate = GeneratedCertificate.eligible_certificates.get(user=user.id, course_id=course_key)
        except GeneratedCertificate.DoesNotExist:
            LOGGER.exception(
                f"Task award_course_certificate was called without a course certificate found for user {user} in "
                f"course run {course_run_key}"
            )
            return

        if certificate and certificate.mode in CourseMode.CERTIFICATE_RELEVANT_MODES:
            ...
            LOGGER.info(
                ...
                f"{course_run_key} with a visible date of {visible_date}"
            )

            # TODO: this should probably be refactored, we should have a utility function for retrieving a date override
            # of a given user/certificate/course-run
            try:
                date_override = certificate.date_override.date
                LOGGER.info(
                    f"Task award_course_certificate will award a course certificate to user {user} in course run"
                    f"{course_run_key} with a date override of {date_override}"
                )
            ...
            LOGGER.info(f"Awarded a course certificate to user {user} in course run {course_run_key}")
    except Exception as exc:
        error_msg = (
            f"Failed to determine course certificates to be awarded for user {user} in course run {course_run_key}"
        )
        LOGGER.exception(error_msg)
        raise _retry_with_custom_exception(
            username=username,
...
def revoke_program_certificates(self, username, course_key):  # lint-amnesty, pylint: disable=too-many-statements
    ...
    try:
        ...
        LOGGER.exception(f"Task revoke_program_certificates was called with invalid username {username}")
        return
    ...
    try:
        ...
        # determine if this user has any program certificates that need to be revoked
        program_uuids_to_revoke = get_revokable_program_uuids(course_specific_programs, student)
    except Exception as exc:
        error_msg = (
            f"Failed to determine program certificates to be revoked for user {student}"
        )
        LOGGER.exception(error_msg)
        raise _retry_with_custom_exception(
            username=username,
    ...
    if program_uuids_to_revoke:
            ...
            try:
                ...
                if exc.response.status_code == 404:
                    LOGGER.exception(
                        ...
                        "program certificate could not be found for the given user in this program"
                    )
                elif exc.response.status_code == 429:
                    rate_limit_countdown = 60
                    error_msg = (
                    ...
                    LOGGER.exception(
                        f"Failed to revoke program certificate from user {student} in program {program_uuid}"
                    )
            except Exception:  # pylint: disable=broad-except
                # keep trying to revoke other certs, but retry the whole task to fix any missing entries
                LOGGER.warning(f"Failed to revoke program from user {student} in program {program_uuid}")
                failed_program_certificate_revoke_attempts.append(program_uuid)

        if failed_program_certificate_revoke_attempts:
            # N.B. This logic assumes that this task is idempotent
            LOGGER.info(f"Retrying task to revoke a program certificate from user {student} in program {program_uuid}")
            # The error message may change on each reattempt but will never be raised until the max number of retries
            # have been exceeded. It is unlikely that this list will change by the time it reaches its maximimum number
            # of attempts.
            error_msg = (
                f"Failed to revoke program certificate(s) for user {student} in program(s) "
        ...
        LOGGER.info(f"There are no program certificates to revoke for user {username}")
...
def update_certificate_available_date_on_course_update(self, course_key):
    ...
    When executed, this task is responsible for enqueuing additional subtasks responsible for updating the CAD value in
    the Credentials IDA's internal records.
