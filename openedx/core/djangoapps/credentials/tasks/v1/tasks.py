...
def backfill_date_for_all_course_runs():
    """
    This task is enqueued by the `update_credentials_available_date` management command.

    It enqueues an `update_certificate_available_date_on_course_update` subtask for each course overview in the system.
    We use an existing Celery task so that we don't have to duplicate the "what date do we send to Credentials" and REST
    API request logic in multiple places.
    """
